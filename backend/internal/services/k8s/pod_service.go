			...
			Containers: []corev1.Container{
				{
					...
					Args:            getContainerArgs(config.Type, config.ContainerPort),
					...
					Lifecycle: podLifecycle(config.Type),
					Resources: resources,
...
// ExecInPod runs a command in the specified pod and returns stdout+stderr.
func (s *PodService) ExecInPod(ctx context.Context, userID, instanceID int, command []string) (string, error) {
	...
	if err := exec.StreamWithContext(ctx, remotecommand.StreamOptions{
		...
		if out != "" || errOut != "" {
			return out + errOut, fmt.Errorf("command failed: %w\noutput: %s", err, out+errOut)
		}
...
// getContainerArgs returns the command arguments for different container types
func getContainerArgs(instanceType string, port int32) []string {
	switch instanceType {
	case "openclaw":
		return []string{
...
// podLifecycle returns a Lifecycle hook for the container.
// For openclaw instances it auto-installs common plugins at pod start.
func podLifecycle(instanceType string) *corev1.Lifecycle {
		...
		PostStart: &corev1.LifecycleHandler{
			Exec: &corev1.ExecAction{
				Command: []string{
					"/bin/sh", "-c",
					"sleep 3 && " +
						"node openclaw.mjs plugins install @openclaw/feishu 2>/dev/null; " +
						"node openclaw.mjs plugins install @tencent-weixin/openclaw-weixin 2>/dev/null; " +
						"true",
				},
			},
		},
