...
This shim bridges NousResearch/hermes-agent with ClawManager's Agent Control Plane.
It runs alongside hermes-agent and handles:
  - Registration with ClawManager
  - Heartbeat reporting
...
Environment Variables (injected by ClawManager):
  ...
  CLAWMANAGER_AGENT_BASE_URL       - ClawManager API base URL (e.g., http://clawmanager-gateway:9001)
...
class ClawManagerShim:
    """Shim for integrating hermes-agent with ClawManager."""
    ...
    def _make_request(self, method: str, path: str, token: Optional[str] = None,
        ...
        """Make HTTP request to ClawManager."""
    ...
    def _get_hermes_pid(self) -> Optional[int]:
        """Try to find hermes-agent process PID."""
        try:
            import subprocess
    ...
    def execute_command(self, command: Dict) -> Dict:
        """Execute a command from ClawManager."""
        ...
        if cmd_type == 'start':
            result = self._cmd_start(params)
        elif cmd_type == 'stop':
            result = self._cmd_stop(params)
        elif cmd_type == 'restart':
            result = self._cmd_restart(params)
        elif cmd_type == 'health_check':
            result = self._cmd_health_check(params)
        elif cmd_type == 'install_skill':
            result = self._cmd_install_skill(params)
        else:
    ...
    def _cmd_start(self, params: Dict) -> Dict:
        ...
        # Hermes-agent doesn't have a simple stop/start via CLI
        # Just report status
        return {'success': True, 'message': 'Hermes-agent runs continuously'}

    def _cmd_stop(self, params: Dict) -> Dict:
        """Handle stop command."""
        return {'success': True, 'message': 'Hermes-agent stop not supported via shim'}

    def _cmd_restart(self, params: Dict) -> Dict:
        """Handle restart command."""
        return {'success': True, 'message': 'Hermes-agent restart not supported via shim'}

    def _cmd_health_check(self, params: Dict) -> Dict:
        """Handle health check command."""
        status = self._get_hermes_status()
        return {'success': True, 'status': status}

    def _cmd_install_skill(self, params: Dict) -> Dict:
        """Handle install_skill command."""
        # Hermes-agent has its own skill system
        # This is a placeholder for future implementation
        return {'success': False, 'message': 'Skill install not yet implemented for hermes-agent'}
    ...
    def _get_skill_count(self) -> int:
        """Get skill count from hermes-agent."""
        # Placeholder - hermes-agent doesn't expose skill count in the same way
        return 0

    def _get_disk_used(self) -> int:
        """Get disk usage of persistent directory."""
        try:
            import shutil
    ...
    def run(self):
        ...
        # Initial registration or re-register if needed
